package com.teenthofabud.laundromat.infrastructure.access.resource.converter;
...
@Component
@Slf4j
public class ResourceDto2EntityConverter implements ComparativePatchConverter<ResourceDto, ResourceEntity> {

    private static final Integer NO_OF_COMPARABLE_AND_MAPPABLE_FIELDS = 4;


    @Override
    public void compareAndMap(ResourceDto dto, ResourceEntity actualEntity) throws TOABBaseException {
        ...
        if(optDescription.isPresent()) {
            ...
            changeSW[i++] = true;
            log.debug("ResourceDto.description is valid");
        }
        Optional<String> optName = dto.getName();
        if(optName.isPresent()) {
            actualEntity.setName(optName.get());
            changeSW[i++] = true;
            log.debug("ResourceDto.name is valid");
        }
        Optional<String> optActive = dto.getActive();
        if(optActive.isPresent()) {
            actualEntity.setActive(Boolean.valueOf(optActive.get()));
            changeSW[i++] = true;
            log.debug("ResourceDto.active is valid");
        }
        if(Collections.frequency(Arrays.asList(changeSW), Boolean.TRUE) >= 1) {
            log.debug("All provided ResourceDto attributes are valid");
            actualEntity.setModifiedOn(LocalDateTime.now(ZoneOffset.UTC));
            return;
        }
        log.debug("Not all provided ResourceDto attributes are valid");
    }
}
